...
const connectionString = "postgresql://postgres.jhjojgjuonmwjgoxzicy:wCUbaBOT4wirD6dF@aws-1-ap-southeast-1.pooler.supabase.com:6543/postgres?pgbouncer=true";
...
async function main() {
  ...
  // Clear existing teams for these two to avoid unique constraint issues
  await prisma.team.deleteMany({
    where: { ownerId: { in: [chien.id, linh.id] } }
  });

  console.log("Tạo Team...");
  const teamChien = await prisma.team.create({
    data: {
  ...
  const teamLinh = await prisma.team.create({
    data: {
  ...
  await prisma.teamMember.create({ data: { teamId: teamChien.id, userId: chien.id, role: 'LEADER' } });
  await prisma.teamMember.create({ data: { teamId: teamLinh.id, userId: linh.id, role: 'LEADER' } });
  ...
  for (let t = 0; t < 2; t++) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      // Cố gắng tìm nếu đã có thì update
